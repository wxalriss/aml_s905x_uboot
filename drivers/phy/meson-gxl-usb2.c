...
static int phy_meson_gxl_usb2_power_on(struct phy_meson_gxl_usb2_priv *priv)
{
	uint val;
...
static void phy_meson_gxl_usb2_power_off(struct phy_meson_gxl_usb2_priv *priv)
{
	uint val;
...
static int phy_meson_gxl_usb2_init(struct phy *phy)
{
	...
	ret = phy_meson_gxl_usb2_power_on(priv);
	if (ret)
		return ret;
...
static int phy_meson_gxl_usb2_exit(struct phy *usb_phy)
{
	struct udevice *dev = usb_phy->dev;
	struct phy_meson_gxl_usb2_priv *priv = dev_get_priv(dev);

	phy_meson_gxl_usb2_power_off(priv);	
