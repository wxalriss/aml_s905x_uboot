...
static void phy_meson_gxl_usb3_power_on(struct phy_meson_gxl_usb3_priv *priv)
{
	uint val;
...
static void phy_meson_gxl_usb3_power_off(struct phy_meson_gxl_usb3_priv *priv)
{
	uint val;
...
static int phy_meson_gxl_usb3_init(struct phy *phy)
{
	...
	phy_meson_gxl_usb3_power_on(priv);

	phy_meson_gxl_usb3_set_host_mode(priv);
...
static int phy_meson_gxl_usb3_exit(struct phy *usb_phy)
{
	struct udevice *dev = usb_phy->dev;
	struct phy_meson_gxl_usb3_priv *priv = dev_get_priv(dev);

	phy_meson_gxl_usb3_power_off(priv);	
