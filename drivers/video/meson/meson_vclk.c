...
static void meson_vclk_set(struct meson_vpu_priv *priv, unsigned int pll_base_freq,
			  ...
			  const struct display_timing *mode, bool is_cvbs)
{
	...
	bool hdmi_use_enci = false;
	int vic = edid_match_cea_mode(mode);
	...
	debug("vclk:%d venc=%d hdmi=%d enci=%d\n",
		vclk_freq, venc_freq, hdmi_freq, hdmi_use_enci);

	meson_vclk_setup(priv, MESON_VCLK_TARGET_HDMI,
			vclk_freq, venc_freq, hdmi_freq, hdmi_use_enci);
}
